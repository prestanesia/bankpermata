...
class BankPermata extends PaymentModule
{
    ...
    public function __construct()
    {
        ...
        $config = Configuration::getMultiple(array('BANK_PERMATA_DETAILS', 'BANK_PERMATA_OWNER', 'BANK_PERMATA_ADDRESS'));
        ...
        $this->displayName = $this->l('Bank Permata');
        $this->description = $this->l('Accept payments for your products via Bank Permata transfer.');
    ...
    public function install()
    {
        ...
        $newState->module_name = 'bankpermata';
        $newState->invoice = false;
        $newState->color = "#099942";
        ...
        foreach ($languages as $language) {
            if ($language['iso_code'] == 'id') {
                $newState->name[(int)$language['id_lang']] = 'Menunggu pembayaran via Bank Permata';
            } else {
                $newState->name[(int)$language['id_lang']] = 'Awaiting Bank Permata Payment';
            }
        ...
        if ($newState->add()) {
            ...
            foreach ($languages as $language) {
                if ($language['iso_code'] == 'id') {
                    copy(dirname(__FILE__).'/mails/id/bankpermata.html', _PS_MAIL_DIR_.'/'.strtolower($language['iso_code']).'/bankpermata.html');
                    copy(dirname(__FILE__).'/mails/id/bankpermata.txt', _PS_MAIL_DIR_.'/'.strtolower($language['iso_code']).'/bankpermata.txt');
                } else {
                    copy(dirname(__FILE__).'/mails/en/bankpermata.html', _PS_MAIL_DIR_.'/'.strtolower($language['iso_code']).'/bankpermata.html');
                    copy(dirname(__FILE__).'/mails/en/bankpermata.txt', _PS_MAIL_DIR_.'/'.strtolower($language['iso_code']).'/bankpermata.txt');
                }
            }
        } else {
            return false;
    ...
    {
        if (!Configuration::deleteByName('BANK_PERMATA_DETAILS') || !Configuration::deleteByName('BANK_PERMATA_OWNER') || !Configuration::deleteByName('BANK_PERMATA_ADDRESS') || !parent::uninstall()) {
            return false;
        }
        return true;
    }
    ...
    public function hookDisplayPaymentEU($params)
    {
        ...
        $payment_options = array(
            'cta_text' => $this->l('Pay by Bank Permata'),
    ...
    public function hookPaymentReturn($params)
    {
        ...
        if (in_array($state, array(Configuration::get('PS_OS_BANKPERMATA'), Configuration::get('PS_OS_OUTOFSTOCK'), Configuration::get('PS_OS_OUTOFSTOCK_UNPAID')))) {
            $this->smarty->assign(array(
    ...
    {
        $this->context->controller->addCSS(($this->_path).'bankpermata.css', 'all');
    }
}
